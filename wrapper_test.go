...
func (s *GrpcWebWrapperTestSuite) SetupSuite() {
	...
	tlsConfig, err := connhelpers.TlsConfigForServerCerts("testdata/localhost.crt", "testdata/localhost.key")
...
func (s *GrpcWebWrapperTestSuite) TestPingEmpty() {
	headers, trailers, responses, err := s.makeGrpcRequest(
		...
		headerWithFlag(),
		serializeProtoMessages([]proto.Message{&empty.Empty{}}))
...
func (s *testServiceImpl) PingEmpty(ctx context.Context, _ *empty.Empty) (*testproto.PingResponse, error) {
...
func (s *testServiceImpl) PingError(ctx context.Context, ping *testproto.PingRequest) (*empty.Empty, error) {
